...
def _backbone_contrast_from_filename(fname: str) -> str | None:
    """`ma_2mo_AppP_ma_2mo_WTyp_backbone_output.parquet` → 'App_2mo'."""
    m = re.match(
        r"ma_(\d+)mo_([A-Za-z]+)_ma_\1mo_WTyp_backbone_output\.parquet$", fname
    )
    if not m:
        return None
    age, geno_token = m.group(1), m.group(2)
...
def _write_incytr_pair_pathways() -> dict | None:
    ...
    # Collect backbone parquet paths for signature (so backbone data changes
    # invalidate the build cache and trigger a full rebuild).
    backbone_input_files: list[str] = []
    for _grain in BACKBONE_GRAIN_NODES:
        _gdir = os.path.join(INCYTR_BACKBONE_PAIR_MODE_DIR, _grain)
    ...
    signature = _input_signature(
         ...
         _incytr_celltype_qc_counts_path(), *backbone_input_files],
        {
            ...
            "score_columns_base": list(_INCYTR_SCORE_COLS_BASE),
            "score_columns_optional": list(_INCYTR_SCORE_COLS_OPTIONAL),
            "backbone_index_file": _BACKBONE_INDEX_FILENAME,
    ...
    dir_flag_cols = [c for c in ("pr_up", "pr_down", "ps_up", "ps_down",
                                  "py_up", "py_down")
                     if c in src_cols]
    # B-5: optional PTM-track score columns present in this cohort's schema.
    # Value-based gating (non-zero check) runs after the view is created.
    ...
    for fpath, contrast in file_to_contrast:
        ...
        # Base non-SiK scores + optional cols present in schema; missing base
        # cols get NULL-filled, missing optional cols are excluded.
        ...
        # Stable column order; missing base scores are NULL-filled.
        # Optional cols not in this file's schema are simply excluded (no NULL fill).
    ...
    # B-5: which optional score cols have any non-zero value in this cohort?
    # Only these are included in the binary index, shards, and score_columns.
    # Cols present but all-zero (e.g. Ack/KGG/Rme1 on Song) are excluded.
    ...
    if active_optional:
        print(f"    score_columns: {len(active_optional)} optional channel(s) active: "
              f"{list(active_optional)}", flush=True)
    else:
        print(f"    score_columns: no optional channels active "
    ...
    # B-5: column layout uses effective_score_cols (base + active optional).
    # Alignment: f4 first, then u2, then u1 — each column's byte offset is a
    # multiple of its element size (zero-copy TypedArray views in JS require this).
    INCYTR_INDEX_COLUMNS = (
        [("PDS", "f4"), ("pvalue", "f4")]
        + [(sc, "u2") for sc in effective_score_cols]   # float16 bit-patterns
    ...
    shard_select_cols = (
        ["Ligand", "Receptor", "EM", "Target",
         "contrast", "pvalue", "PDS"]
        + list(effective_score_cols)   # B-5: base + active optional only
        + dir_flag_cols
        + extra_path_cols
        + fc_select
        + label_select
    ...
    payload_block = {
        "schema_version": SCHEMA_VERSION,
        "version": 4,   # v4: backbone_grains (B-3/B-4) + effective score cols (B-5)
        ...
        **({"celltype_groups": celltype_groups} if celltype_groups else {}),
        ...
        "slice_index": index,
        "score_columns": list(effective_score_cols),   # B-5: base + active optional
        "label_columns": list(_INCYTR_LABEL_COLS),
        "label_nodes": list(_INCYTR_LABEL_NODES),
        "label_vocab": list(_INCYTR_LABEL_VOCAB),
        ...
        # Absent when backbone parquets have not been produced (e.g. sce4 parity runs
        # that set BACKBONE_OUT_DIR="" to disable backbone emission).
