...
| `tcell_within_cohort.py` | Within-cohort attribution for T-cell donor1: localizes the bulk kinase MEA to the evidence-backed per-cell states using the cohort's paired scRNA. Emits `tcell_enrichment.csv`, `tcell_concordance.csv`, and ungated `unified_attribution_tcells.csv` under `outputs/reports/kinase_attribution_tcells/donor1/`. CLI: `pixi run tcell-within-cohort`. |
