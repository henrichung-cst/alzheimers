...
if [[ ! -d "$SEA_AD_DIR" ]] || [[ -z "$(ls -A "$SEA_AD_DIR" 2>/dev/null)" ]]; then
  ...
  bash alz/runners/supporting/run_atlas_reference.sh 2>&1 | tee -a "$LOG"
fi
...
if [[ "$N_H5AD" -lt 13 ]]; then
  ...
  bash alz/runners/supporting/run_wmb_download.sh 2>&1 | tee -a "$LOG"
fi
...
if [[ ! -f "$WMB_KINASE" ]] || [[ ! -f "$WMB_PROTEOME" ]]; then
  ...
  bash alz/runners/supporting/run_wmb_expression.sh 2>&1 | tee -a "$LOG"
fi
...
if [[ ! -f "$SONG_SPEC" ]] || [[ ! -f "$SONG_CONC" ]]; then
  ...
  bash alz/runners/supporting/run_snrna_integration.sh 2>&1 | tee -a "$LOG"
fi

note "Running data ingestion"
bash alz/runners/main/run_data_ingest.sh 2>&1 | tee -a "$LOG"

note "Running kinase attribution"
bash alz/runners/main/run_kinase_attribution.sh 2>&1 | tee -a "$LOG"

note "Running attribution recovery"
bash alz/runners/main/run_attribution_recovery.sh 2>&1 | tee -a "$LOG"
