...
$PYTHON alz/bulk_mea/normalize.py 2>&1 | tee -a "$LOG"

echo "--- Stage 2/3: OLS Models + MEA Kinase Enrichment ---" | tee -a "$LOG"
$PYTHON alz/bulk_mea/enrich.py 2>&1 | tee -a "$LOG"

echo "--- Stage 3/3: Unified Cell-Type Attribution ---" | tee -a "$LOG"
$PYTHON alz/bulk_mea/attribute.py 2>&1 | tee -a "$LOG"

echo "=== Kinase attribution finished at $(date) ===" | tee -a "$LOG"
echo "Summary:"
$PYTHON alz/bulk_mea/summary.py
