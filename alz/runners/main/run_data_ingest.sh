...
$PYTHON alz/ingest/song.py --mapping 2>&1 | tee -a "$LOG"

echo "--- Step 2/4: Phosphosite-to-Protein Matching ---" | tee -a "$LOG"
$PYTHON alz/ingest/song.py --phospho-match 2>&1 | tee -a "$LOG"

echo "--- Step 3/4: Marker Protein Assessment (WMB atlas) ---" | tee -a "$LOG"
$PYTHON alz/ingest/song.py --markers 2>&1 | tee -a "$LOG"

echo "--- Step 4/4: Data Quality Assessment ---" | tee -a "$LOG"
$PYTHON alz/ingest/song.py --quality 2>&1 | tee -a "$LOG"

echo "=== Data ingestion finished at $(date) ===" | tee -a "$LOG"
echo "Summary:"
$PYTHON alz/ingest/song.py --summary
