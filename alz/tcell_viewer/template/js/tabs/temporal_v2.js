// ---------------------------------------------------------------------------
// Legacy unified temporal series builder.
// ---------------------------------------------------------------------------
...
function renderTemporalV2() {
  ...
  el.on && el.on("plotly_click", ev => {
    ...
    const ids = cd[1];
    const label = `Temporal · ${_tv2SeriesLabel(ser)} · ${meta.g}_${p.x}` +
...
function _openKinaseDeepDiveWithWhitelist(kinaseIds, sourceLabel, ctx) {
  ...
  // and edit the implied scope. T-cell kinase MEA scopes by day contrast; the
  // legacy disease/timepoint/celltype slots are kept only for shared handoff
  // compatibility and do not drive attribution filters without attribution rows.
  if (ctx) {
    const day = ctx.genotype && CONTRASTS.indexOf(ctx.genotype) >= 0
      ? ctx.genotype
      : (ctx.timepoint && CONTRASTS.indexOf(ctx.timepoint) >= 0 ? ctx.timepoint : "");
    const patch = {
      day:        day ? [day] : [],
      disease:    [],
      timepoint:  [],
