...
//
// Two orthogonal specificity axes:
//   cell-TYPE specificity (CD8/CD4/Treg) drives the confidence pill (kinase-level)
...
//   · tcell_state_enrichment (State specificity, gated on detection) · tcell_lfc
//   (Δ vs d2 — info) · tcell_concordance (vs Bulk — info) · tcell_consistency
//   (Timecourse — info) · nsclc_detected (NSCLC Detected)
//
// Sections (accordion, in order):
//   §0 Specificity verdict (cell-type axis sets the pill; state axis is info;
//      NSCLC corroborator)
//   §1 Within-cohort transcript trace (local)
//   §2 NSCLC lineage strip (local)
...
const TCELL_MANIFEST = (() => {
  ...
  // median state across the full ProjecTILs state set, so
  // it is not saturated by the transcriptional homogeneity of ProjecTILs states
  // (where a dominance-of-total concentration tier cannot localize). Reuses the
  // shared badge palette (vhi/hi/mid/lo). Local to T-cell (no other cohort has
  // this axis).
  function _tcellEnrichCell(fold) {
    ...
    if (!cls) {
      return `<span class="muted" title="Within ~1.5× of the typical (median) T-cell state — broadly expressed, not state-enriched">${lbl}</span>`;
    }
    return `<span class="badge ${cls}" title="${lbl} the kinase's median-state expression — enriched in this state relative to a typical T-cell state">${lbl}</span>`;
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      title: "Kinase-level cell-type specificity: how concentrated this kinase is in one T-cell TYPE (CD8 / CD4 / Treg), from the donor's own scRNA, corroborated by the independent NSCLC reference detecting the kinase in its T_NK lineage. tier ≥2 (≥2× the even 1/3 share) → moderate/high; broad → low. Shown once, on the kinase's headline state row — it is a per-kinase property, not per state.",
    ...
    },
    {
      ...
      title: "Is the kinase's transcript detected in this state? ✓/✗ = fraction of cells expressing ≥ 1% (normalization-free presence), with the % shown. Sorts by detection.",
      render(r, _numFmt, _ctx) {
    ...
    },
    {
      ...
      render(r, _numFmt, _ctx) {
        ...
        return `<td class="attr-num">${det ? _tcellEnrichCell(r.tcell_state_enrichment)
          : '<span class="muted" title="Not detected in this state (fraction < 1%) — no state enrichment">—</span>'}</td>`;
      },
    },
    {
    ...
    },
    {
      ...
      title: "Independent reference (10x 897k-cell NSCLC): is the kinase actually present in THIS exact T-cell state? Fraction of reference cells expressing it (✓ = expressed in ≥1 cell — no minimum-fraction floor; the reference is deep enough that any nonzero fraction is real). n/a = the kinase is outside the NSCLC probe panel. Sorts by detection.",
      render(r, _numFmt, _ctx) { return _nsclcDetCell(r.nsclc_frac, r.nsclc_detected); },
    },
  ];
  ...
  function superGroupRow(cols) {
    ...
    return `<tr class="attr-verdict-supergroup">` +
      ...
      `<th class="attr-supergroup-attr" colspan="${(_grpCounts.attr || 0)}" title="Within-cohort cell-state attribution: transcript specificity across this cohort's own ProjecTILs T-cell states + concordance vs the bulk kinase-MEA direction at this contrast.">Within-cohort attribution (vs bulk direction)</th>` +
      `<th class="attr-supergroup-ext" colspan="${(_grpCounts.ext || 0)}" title="Independent human reference: 10x 897k-cell NSCLC (ProjecTILs/scGate T-states + marker-labeled non-T lineages). Detection at the matched T-state — is the kinase actually present where the within-cohort attribution localized it?">NSCLC reference (detection)</th>` +
    `</tr>`;
  }
  ...
  // tooltip; the §0 specificity verdict section provides the full breakdown).
  ...
  const explainerHtml =
    ...
    `<table class="attr-explainer-table" style="margin-bottom:8px;">` +
      ...
      `<tr><td><strong>Detected</strong></td><td>Within-cohort detection in this cohort's own scRNA, pooled across all days: fraction ≥ 1% (normalization-free presence).</td></tr>` +
      `<tr><td><strong>Specificity</strong></td><td>State enrichment — this state's transcript as a fold over the kinase's <em>median</em> state across all ProjecTILs states. The T-cell specificity metric asks "how many fold above a typical T-cell state?" and is not flattened by the transcriptional homogeneity of ProjecTILs states (where a dominance-of-total concentration tier saturates and cannot localize). ≥3× strong / ≥2× moderate / ≥1.5× mild; &lt;1.5× = broadly expressed.</td></tr>` +
      `<tr><td><strong>Δ vs d2 / vs Bulk</strong></td><td>Does the transcript move the same direction as the bulk kinase activity at this day? <strong>Info only</strong> — concordance between kinase activity and its own mRNA is at chance (OR≈1, verified 2026-06-03; same in the published mouse Song method). Never used to filter.</td></tr>` +
      ...
      `<tr><td><strong>NSCLC Detected</strong></td><td>Independent 10x 897k-cell NSCLC reference: fraction of cells in this exact T-state that express the kinase (✓ = expressed in ≥1 cell — no minimum-fraction floor). An independent corroborator of the within-cohort attribution. n/a = outside the NSCLC probe panel.</td></tr>` +
      `</tbody></table>` +
  ...
  // ---- §0 Specificity verdict — T-cell local renderer ----------------------
  ...
  function _renderTcellSpecificityVerdict(secHostId, ctx, row) {
    const host = document.getElementById(secHostId);
    if (!host) return;
    const conf = row.confidence_tier || "none";
    const basis = row.confidence_basis || "";
    // tcell_top_celltype is now the cell TYPE home (CD8 / CD4 / Treg) — the axis
    // the confidence pill is computed on. tcell_effective_n is the per-STATE
    // subtype spread (info only; it does NOT set the confidence pill).
    const ctHome = row.tcell_top_celltype || "";
    ...
    // per-state fold-over-median across the kinase's ELIGIBLE states (ineligible
    ...
    const thisCellDlHtml =
      `<dt>Cells expressing</dt><dd>${_specPct(row.tcell_fraction_expressing)}${detected ? "" : ' <span class="muted">(below 1%; not eligible for state enrichment)</span>'}</dd>` +
      `<dt>State enrichment</dt><dd>${detected ? _tcellEnrichCell(row.tcell_state_enrichment) : '<span class="muted" title="Not detected in this state — no state enrichment">—</span>'} <span class="muted">vs the kinase's typical (median) state</span></dd>` +
      `<dt>NSCLC corroborator</dt><dd>${corrLine}</dd>`;

    const reconcileSentence =
      `The confidence pill reflects <strong>cell-type specificity</strong> (CD8 / CD4 / Treg), ` +
    ...
    const kinaseDlHtml =
      `<dt>Cell-type home</dt><dd>${_escapeHtml(ctHome) || "<span class='muted'>—</span>"} <span class="muted">— the type (CD8 / CD4 / Treg) it concentrates in; sets the confidence pill</span></dd>` +
      ...
      `<dt>Subtype spread</dt><dd><span class="badge ${band.badge}">${_specF2(eff)}</span> <span class="muted">effective # ProjecTILs states (info; not the confidence axis)</span></dd>` +
  ...
  function _renderTcellTranscriptTrace(secHostId, ctx, row) {
    ...
    if (cellRows.length === 0) {
      ...
      traceBody =
        ...
        `(detection shown separately; state enrichment = fold over the kinase's median state). ` +
  ...
  // §0 specificity verdict (T-cell-local renderer, NSCLC corroborator)
  // §1 within-cohort transcript trace (T-cell-local)
  // §2 NSCLC lineage strip (references the local _renderNSCLCLineageStrip
  ...
  const sections = [
    {
      ...
      render(secHostId, ctx, row, _kstats) {
        _renderTcellSpecificityVerdict(secHostId, ctx, row);
      },
    },
    {
      id: "transcript-trace",
    ...
    },
    {
      ...
      render(secHostId, ctx, _row, _kstats) {
        // _renderNSCLCLineageStrip is defined in kinase_audit.js (T-cell-specific,
        // not shared). It reads ctx.nsclcRows which is loaded by _loadKinaseAuditContext.
        if (typeof _renderNSCLCLineageStrip === "function") {
          _renderNSCLCLineageStrip(secHostId, ctx);
        } else {
          const host = document.getElementById(secHostId);
          if (host) host.innerHTML = `<div class="muted">NSCLC lineage strip unavailable.</div>`;
        }
      },
    },
  ];
  ...
  return {
    ...
    // buildKstats omitted — T-cell specificity verdict is T-cell-local.
