...
const METRIC_DEFS = {
  ...
  incytrPathways:{ label: "Incytr pathways", short: "Observed-edge pathway participation (mirrors the sidechain graph): count and % of all predicted pathways with at least one contrast- and receiver-matched observed phospho-change kinase→node edge landing on Receptor, EM, or Target. Terminal edges carry no Ligand role, so unlike the prior substrate-based rule, Ligand does not contribute. Denominator is the donor's full predicted-pathway universe (percentages are small by construction). 0 = no observed pathway edge." },
  incytrBackbones:{ label: "Incytr backbones", short: "Observed-edge backbone participation (mirrors the sidechain graph): count and % of all predicted pathways with at least one contrast- and receiver-matched observed phospho-change kinase→node edge landing on Receptor or EM (Target excluded). Terminal edges carry no Ligand role, so unlike the prior substrate-based rule, Ligand does not contribute. Denominator is the donor's full predicted-pathway universe (percentages are small by construction). 0 = no observed pathway edge." },
