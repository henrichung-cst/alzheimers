...
**Verification note:** Rebuild with `pixi run python alz/build_unified_viewer.py` then hard-reload (Ctrl+Shift+R). Open the reference pathway `Apoe|App|Stk11|Cttnbp2` (Astrocytes → Basal-Ganglia-GABAergic-Neurons, ApTt_2mo), expand any row, confirm the Evidence tab is the default, and confirm 4 node columns with 4 layer sub-rows each. Stk11 (EM) should show Basal-Ganglia cluster values (receiver cluster) — not Astrocytes (sender).
