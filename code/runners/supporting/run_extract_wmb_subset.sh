...
#
# Usage:
#   bash code/runners/supporting/run_extract_wmb_subset.sh
set -euo pipefail
...
if [[ ! -f "$GENE_LIST" ]]; then
    ...
    echo "Run first: python code/data_ingest.py --phospho-match"
    exit 1
fi
...
if [[ "$N_H5AD" -lt 13 ]] && [[ "$N_ZST" -gt 0 ]]; then
    ...
    bash code/runners/supporting/decompress_atlas_cache.sh WMB
...
python code/runners/supporting/extract_wmb_gene_subset.py
...
if [[ "$NEED_RECOMPRESS" == true ]]; then
    ...
    if [[ -f "code/runners/supporting/compress_atlas_cache.sh" ]]; then
        bash code/runners/supporting/compress_atlas_cache.sh WMB
    else
