...
Joins each MEA row (keyed on wmb_class × contrast × kinase) to the kinase's
own gene LFC in ``song_concordance.csv`` (males-only factorial OLS at
WMB-class level). Adds:
...
def annotate(mea_df: pd.DataFrame) -> pd.DataFrame:
    """Join MEA table with snRNA gene LFC; return augmented DataFrame.

    `mea_df` must already carry `wmb_class`, `contrast`, `kinase`, `NES`, `FDR`.
    """
